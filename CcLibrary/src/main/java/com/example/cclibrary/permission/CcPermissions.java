...
/*
       //*****************************  TheInstance *****************************
                ...
                CcPermissions.with(getActivity()).permission(PermissionName.CAMERA).request(new Consumer(){
                    @Override
...
public class CcPermissions {
    ...
    private CcPermissions(FragmentActivity activity) {
        mActivity = activity;
    }

    public Activity getSingleContext() {
        return mActivity;
    }
     ...
     * 设置请求的对象
     */
    public static CcPermissions with(FragmentActivity activity) {
