...
public class PermissionUtils {
    ...
    static ArrayList<String> getFailPermissions(Context context, List<String> permissions) {
        ...
        for (String permission : permissions) {
            ...
            //检测安装权限
            if (permission.equals(PermissionName.REQUEST_INSTALL_PACKAGES)) {
            ...
            //检测悬浮窗权限
            if (permission.equals(PermissionName.SYSTEM_ALERT_WINDOW)) {
            ...
            //检测8.0的两个新权限
            if (permission.equals(PermissionName.ANSWER_PHONE_CALLS) || permission.equals(PermissionName.READ_PHONE_NUMBERS)) {
     ...
     * @param permissions            请求的权限
     */
    static String getCheckDeniedStr(Activity activity, List<String> permissions) {
    ...
    static boolean checkSinglePermissionPermanentDenied(Activity activity, String permission) {
        ...
        if (permission.equals(PermissionName.REQUEST_INSTALL_PACKAGES) || permission.equals(PermissionName.SYSTEM_ALERT_WINDOW)) {
            return false;
        }
        ...
        if (manifest != null && manifest.size() != 0) {
            for (String permission : requestPermissions) {
                if (!manifest.contains(permission)) {
                    throw new ManifestRegisterException("检测权限有没有在清单文件中注册"+permission);
                }
            }
        } else {
    ...
    static void checkTargetSdkVersion(Context context, List<String> requestPermissions) {
        //检查是否包含了8.0的权限
        if (requestPermissions.contains(PermissionName.REQUEST_INSTALL_PACKAGES)
                || requestPermissions.contains(PermissionName.ANSWER_PHONE_CALLS)
                || requestPermissions.contains(PermissionName.READ_PHONE_NUMBERS)) {
            //必须设置 targetSdkVersion >= 26 才能正常检测权限
            if (context.getApplicationInfo().targetSdkVersion < Build.VERSION_CODES.O) {
                throw new RuntimeException("必须设置 targetSdkVersion >= 26 才能正常检测权限-"+"The targetSdkVersion SDK must be 26 or more");
            }
        }else {
            //必须设置 targetSdkVersion >= 23 才能正常检测权限
            if (context.getApplicationInfo().targetSdkVersion < Build.VERSION_CODES.M) {
                throw new RuntimeException("必须设置 targetSdkVersion >= 23 才能正常检测权限-"+"The targetSdkVersion SDK must be 23 or more");
            }
        }
    }
}
